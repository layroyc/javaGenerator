...
// 查-- 1对多 如果 报错 可以删除啊
@RequestMapping("/select${uppercaseEnity}And${moreEntityUpper}")
public Map select${uppercaseEnity}And${moreEntityUpper}(${uppercaseEnity} ${lowcaseEnity}){

List<${uppercaseEnity}> ${lowcaseEnity}List = ${lowcaseEnity}Service.select${uppercaseEnity}And${moreEntityUpper}(${lowcaseEnity});
Map map=new HashMap();
map.put("${lowcaseEnity}List",${lowcaseEnity}List);
map.put("code",200);
map.put("msg","请求成功");
return map;
}
// 万能查....
@RequestMapping("/select${uppercaseEnity}With表1表2和service一样ByPage")
public Map select${uppercaseEnity}With表1表2和service一样ByPage(${uppercaseEnity} ${lowcaseEnity}, @RequestParam(value = "page", required = false, defaultValue = "1") Integer page, @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize) {


PageHelper.startPage(page,pageSize);
// 多个map 查询
List<Map> ${lowcaseEnity}List = studentService.selectStudentWithClssMajorSchool(${lowcaseEnity});
        PageInfo pageInfo=new PageInfo(${lowcaseEnity}List);
        long total = pageInfo.getTotal();
        Map responseMap=new HashMap<>();
        responseMap.put("pageInfo",pageInfo);
        responseMap.put("code",0);
